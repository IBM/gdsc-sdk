...
public class Workflowv3CreateProductEntityResponse {
  ...
  public Workflowv3CreateProductEntityResponse result(@javax.annotation.Nullable String result) {
    this.result = result;
    return this;
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Workflowv3CreateProductEntityResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in Workflowv3CreateProductEntityResponse is not found in the empty JSON string", Workflowv3CreateProductEntityResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Workflowv3CreateProductEntityResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Workflowv3CreateProductEntityResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
