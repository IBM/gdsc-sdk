...
public class Ecosystemv3CreateDatasetResponse {
  ...
  public Ecosystemv3CreateDatasetResponse status(@javax.annotation.Nullable GooglerpcStatus status) {
    this.status = status;
    return this;
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Ecosystemv3CreateDatasetResponse.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in Ecosystemv3CreateDatasetResponse is not found in the empty JSON string", Ecosystemv3CreateDatasetResponse.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Ecosystemv3CreateDatasetResponse.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Ecosystemv3CreateDatasetResponse` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
