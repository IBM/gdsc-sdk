...
public class Tenantuserv3GetUserNamesRequest {
  ...
  public Tenantuserv3GetUserNamesRequest userIds(@javax.annotation.Nullable List<String> userIds) {
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!Tenantuserv3GetUserNamesRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in Tenantuserv3GetUserNamesRequest is not found in the empty JSON string", Tenantuserv3GetUserNamesRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!Tenantuserv3GetUserNamesRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `Tenantuserv3GetUserNamesRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
