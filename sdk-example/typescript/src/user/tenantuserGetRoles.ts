import { AuthMethodsConfiguration, ServerConfiguration, Middleware, RequestContext, ResponseContext, createConfiguration, TenantuserApi } from '@guardium/ibm-gdsc-sdk-software';
...
// Use configuration 
const apiInstance = new TenantuserApi(config)

apiInstance.tenantuserGetRoles().then((data:any) => {
  console.log('API called successfully. Returned data: ' + JSON.stringify(data));
}).catch((error:any) => console.error(error));

const getRoles = async () => {
    const response = await apiInstance.tenantuserGetRoles();
