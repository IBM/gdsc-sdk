import { AuthMethodsConfiguration, ServerConfiguration, Middleware, RequestContext, ResponseContext, createConfiguration, Reportsv3HeaderType, ReportsRunnerApi, Reportsrunnerv3RunReportRequest} from '@ibm-gdsc/ibm-gdsc-sdk-software';
...
// Use configuration 
const apiInstance = new ReportsRunnerApi(config);

const runReport = async (request: Reportsrunnerv3RunReportRequest ) => {
    const response = await apiInstance.reportsRunnerRunReport(request);
    console.log('API called successfully. Returned data: ' + JSON.stringify(response));
  
}

const request: Reportsrunnerv3RunReportRequest  = {
    reportId: '000000000000000000002001',
    fetchSize: 500,
    offset: 0,
    runtimeParameterList:[
        {
          key: "QUERY_FROM_DATE",
          runtimeParameterType: Reportsv3HeaderType.Date,
